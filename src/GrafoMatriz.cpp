...
int** GrafoMatriz::Matriz = nullptr;  // Declaração da variável estática
...
GrafoMatriz::GrafoMatriz() {
    ...
    if (Matriz == nullptr) {
        Matriz = new int*[tamanhoAtual];  // Aloca a matriz de adjacência dinamicamente
        for (int i = 0; i < tamanhoAtual; i++) {
            Matriz[i] = new int[tamanhoAtual]{0};  // Inicializa com 0
        }
    }

    if (MatrizLinear == nullptr) {
        MatrizLinear = new int[tamanhoAtualLinear]{0};  // Inicializa a matriz linear
    }

    if (VetorPesosVertices == nullptr) {
        VetorPesosVertices = new int[tamanhoAtual]{0};  // Inicializa o vetor de pesos dos vértices
    }
}
