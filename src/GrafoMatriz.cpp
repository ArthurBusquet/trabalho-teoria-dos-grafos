...
int GrafoMatriz::calcularIndiceLinear(int origem, int destino) {
    if (origem <= destino) {
        return (destino * (destino - 1)) / 2 + origem;  // Calcula índice linear considerando origem e destino
    }
    return (origem * (origem - 1)) / 2 + destino;  // Caso contrário, calcula de forma inversa
}
